...
const ClassBoard: React.FC<{ classId: string; userRole: 'teacher' | 'student'; content: any[] }> = ({ 
  ...
  // для режима модалки больше не используем; инлайн-редактор ниже
  ...
  const startDrawing = (e: React.MouseEvent) => {
    ...
    if (currentTool === 'pen') {
      ...
      // При клике на холст добавляем текст в точке клика и открываем инлайн-редактор
      const id = crypto.randomUUID();
      const fontSize = Math.max(12, brushSize * 4);
      const newItem: BoardItem = { id, type: 'text', x, y, color: brushColor, fontSize, text: 'Введите текст', width: 240, height: fontSize + 6, fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'none', locked: false };
      ...
      setSelectedId(id);
      const scale = Math.max(0.01, zoom / 100);
      setTextEditor({ visible: true, itemId: id, value: 'Введите текст', screenX: x * scale + canvasOffset.x, screenY: y * scale + canvasOffset.y });
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (tool.id === 'text') {
                    // При выборе текста сразу создаём текстовый блок по центру и открываем инлайн‑редактор
                    ...
                    const newItem: BoardItem = { id, type: 'text', x: centerX, y: centerY, color: brushColor, fontSize, text: 'Введите текст', width: 300, height: fontSize + 8, fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'none', locked: false };
                    const nextItems = [...items, newItem];
                    setItems(nextItems);
                    ctx.putImageData(history[historyIndex], 0, 0);
                    drawItems(ctx, nextItems as any);
                    saveToHistory();
                    setSelectedId(id);
                    const scale = Math.max(0.01, zoom / 100);
                    const screenX = centerX * scale + canvasOffset.x;
                    const screenY = centerY * scale + canvasOffset.y;
                    setTextEditor({ visible: true, itemId: id, value: 'Введите текст', screenX, screenY });
                  }
                }}
