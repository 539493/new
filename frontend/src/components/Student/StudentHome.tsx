  ...
  React.useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // УБРАНО: повторная попытка для предотвращения бесконечного цикла
      }
    };
    
    loadData();
    
    // УБРАНО: периодическая загрузка каждые 15 секунд для предотвращения бесконечного цикла
    // Данные обновляются через WebSocket события и явные действия пользователя
    
  }, [loadRegisteredTeachers]); // Убрана зависимость forceSyncData для предотвращения цикла
  const { user } = useAuth();
