...
const StudentHome: React.FC<StudentHomeProps> = ({ setActiveTab }) => {
  ...
  React.useEffect(() => {
    ...
    // Принудительная загрузка с задержкой для гарантии инициализации
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            await loadRegisteredTeachers();
            await forceSyncData();
            console.log('✅ Данные загружены при повторной попытке');
          } catch (retryError) {
    ...
    // Устанавливаем периодическую загрузку каждые 15 секунд для гарантии видимости
    const interval = setInterval(() => {
      if (document.visibilityState === 'visible') {
        console.log('⏰ StudentHome: Периодическая загрузка зарегистрированных преподавателей и слотов...');
        loadData();
      }
    }, 15000); // 15 секунд - более стабильное обновление
