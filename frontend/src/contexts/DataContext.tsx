...
export const DataProvider: React.FC<DataProviderProps> = ({ children }) => {
  ...
  const refreshAllData = () => {
    ...
    // УБРАНО: принудительная синхронизация для предотвращения бесконечного цикла
    // Синхронизация происходит через WebSocket события и явные действия пользователя
  };
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      setIsConnected(true);
        ...
        // При подключении запрашиваем данные через WebSocket (без принудительного обновления)
        console.log('🔄 Первоначальное подключение - запрашиваем данные...');
    ...
    newSocket.on('disconnect', () => {
      ...
      newSocket.on('reconnect', (attemptNumber) => {
        ...
        // При восстановлении соединения запрашиваем данные через WebSocket
        console.log('🔄 Восстановление соединения - запрашиваем данные...');
        if (socketRef.current) {
          socketRef.current.emit('requestAllSlots');
          socketRef.current.emit('requestAllUsers');
        }
    ...
    newSocket.on('slotCreated', (newSlot: TimeSlot) => {
      ...
      // УБРАНО: принудительное обновление данных после создания слота для предотвращения бесконечного цикла
    ...
    // (dedup) Слушатель allSlots уже зарегистрирован выше
      ...
      newSocket.on('userRegistered', (newUser: User) => {
        ...
        try {
          // УБРАНО: loadRegisteredTeachers() для предотвращения бесконечного цикла
          if (socketRef.current && isConnected) {
            socketRef.current.emit('requestAllUsers');
          }
        } catch {}
  ...
  useEffect(() => {
    ...
    // УБРАНО: принудительная синхронизация при изменении timeSlots для предотвращения бесконечного цикла
    // Синхронизация происходит только при явных действиях пользователя или WebSocket событиях
  }, [timeSlots]);
