...
const TeacherSearch: React.FC<TeacherSearchProps> = ({ onTeacherSelect, onBookLesson }) => {
  ...
  React.useEffect(() => {
    console.log('🔍 TeacherSearch: allUsers изменились:', allUsers?.length || 0);
    if (allUsers && allUsers.length > 0) {
      const teachers = allUsers.filter((u: any) => u.role === 'teacher');
      console.log('👨‍🏫 TeacherSearch: Обнаружены репетиторы в allUsers:', teachers.length);
  ...
  const filteredTeachers = useMemo(() => {
    ...
    let filtered = teachers.filter(teacher => {
      ...
      // (изменено по запросу пользователя - репетиторы должны быть видны всегда)
